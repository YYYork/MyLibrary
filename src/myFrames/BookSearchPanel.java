	...
	private void addListener() {//给按钮添加监听
		btn_Search_Name.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(bookName.equals(""))
					return;
				Book getbooks[] = BookFinder.getBooks(BookFinderType.SEARCH_FOR_NAME, bookName);//设置表格模型
				if(!getbooks.equals(null)) {
				Books = BookToArrays.booktoArrays(getbooks);
				}else {
					JOptionPane.showMessageDialog(LoginFrame.instance, "没有查询到您想要的书！");//弹出小对话框
				}
			}
		});
		...
		btn_Search_Publisher.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Books = BookToArrays.booktoArrays(getbooks);
					...
					JOptionPane.showMessageDialog(LoginFrame.instance, "没有查询到您想要的种类！");//弹出小对话框
				}
			}
		});
		...
		btn_Search_Type.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Books = BookToArrays.booktoArrays(getbooks);
					...
					JOptionPane.showMessageDialog(LoginFrame.instance, "没有查询到您想要的出版社！");//弹出小对话框
				}
			}
		});
	}
